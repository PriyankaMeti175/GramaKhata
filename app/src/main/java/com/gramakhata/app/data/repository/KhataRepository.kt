...
class KhataRepository(context: Context) {
    ...
    private val sessionManager = com.gramakhata.app.data.SessionManager(context)

    private fun getCurrentUserId(): String = sessionManager.getUserPhone() ?: ""

    // Customer operations
    fun getAllActiveCustomers(): LiveData<List<Customer>> = customerDao.getAllActiveCustomers(getCurrentUserId())
    fun getAllCustomers(): LiveData<List<Customer>> = customerDao.getAllCustomers(getCurrentUserId())
    fun getCustomerById(id: Long): LiveData<Customer> = customerDao.getCustomerById(id)
    fun searchCustomers(query: String): LiveData<List<Customer>> = customerDao.searchCustomers(getCurrentUserId(), query)
    fun getTotalDues(): LiveData<Double?> = customerDao.getTotalDues(getCurrentUserId())
    fun getActiveDebtorCount(): LiveData<Int> = customerDao.getActiveDebtorCount(getCurrentUserId())

    suspend fun addCustomer(customer: Customer): Long {
        return customerDao.insert(customer.copy(userId = getCurrentUserId()))
    }
